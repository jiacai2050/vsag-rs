...
const CError* create_index(const char* in_index_type, const char* in_parameters, void** out_index_ptr) {
    if (!in_index_type || !in_parameters || !out_index_ptr) {
        return new CError{static_cast<int>(vsag::ErrorType::INVALID_ARGUMENT), "Invalid null argument."};
    }
    ...
    if (!result.has_value()) {
        ...
        return new CError{static_cast<int>(result.error().type), strdup(result.error().message.c_str())};
    }
...
const CError* build_index(
    ...
    // Cast the void pointer back to the original pointer type, std::shared_ptr<Index>*
    auto pIndex = static_cast<std::shared_ptr<vsag::Index>*>(in_index_ptr);

    auto base = vsag::Dataset::Make();
    base->NumElements(in_num_vectors)->Dim(in_dim)->Ids(in_ids)->Float32Vectors(in_vectors)->Owner(false);
...
const CError* knn_search_index(
    ...
    if (!in_index_ptr || !in_query_vector || !in_search_parameters || !out_ids || !out_distances || !out_num_results) {
        return new CError{static_cast<int>(vsag::ErrorType::INVALID_ARGUMENT), "Invalid null argument."};
    }
...
void free_error(const CError* error) {
    if (error) {
        free(const_cast<char*>(error->message));  // Properly deallocate the dynamically allocated message
...
void free_index(void* index_ptr) {
    if (index_ptr) {
        // Cast the void pointer back to the original pointer type, std::shared_ptr<Index>*
        std::shared_ptr<vsag::Index>* pIndex = static_cast<std::shared_ptr<vsag::Index>*>(index_ptr);
        ...
        // the decrement of the reference count and will delete the managed Index object
